...
`vercel.json` 已宣告每分鐘的 Vercel Cron（`/api/cron/monitor`），但 **Vercel Hobby（免費）方案的 Cron 只能每天執行一次**，對交易監控不夠。兩個選擇：

- **Vercel Pro**：內建 Cron 即可每分鐘執行，Vercel 會自動帶上 `CRON_SECRET`。
- **免費做法**：用外部排程服務（[cron-job.org](https://cron-job.org)、UptimeRobot 等）每分鐘呼叫：
  ```
